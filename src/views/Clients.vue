<template>
    <div class="flex justify-between items-center mb-3">

     <h1 class="text-xl text-gray-800 font-semibold">Clientes</h1>     
     <select class="border border-gray-300 p-1 rounded w-[150px] text-gray-800 bg-white"
     >
       <option value="2024">2024</option>
       <option value="2025">2025</option>

     </select>
    </div>

      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
    <CardTop 
    cardTitleColor="text-transparent bg-clip-text bg-green-500 bg-gradient-to-r to-blue-600"
    cardLineColor="bg-green-400 bg-gradient-to-r to-blue-600"
    >
      <template v-slot:mainInfo>
        {{ clientStore.clients_count }}
      </template>

      <template v-slot:subInfo>
        Clientes
      </template>
    </CardTop>

    <CardTop 
    cardTitleColor="text-transparent bg-clip-text bg-indigo-500 bg-gradient-to-r to-purple-600"
    cardLineColor="bg-indigo-400 bg-gradient-to-r to-purple-600"
    >
      <template v-slot:mainInfo>
       {{ clientStore.clients_active }}
      </template>

      <template v-slot:subInfo>
        Clientes ativos
      </template>
    </CardTop>

    <CardTop 
    cardTitleColor="text-transparent bg-clip-text bg-orange-500 bg-gradient-to-r to-red-600"
    cardLineColor="bg-orange-400 bg-gradient-to-r to-red-600"
    >
      <template v-slot:mainInfo>
       {{ clientStore.clients_due }}
      </template>

      <template v-slot:subInfo>
        Clientes vencidos
      </template>
    </CardTop>

    <CardTop 
    cardTitleColor="text-transparent bg-clip-text bg-sky-500 bg-gradient-to-r to-indigo-600"
    cardLineColor="bg-sky-400 bg-gradient-to-r to-indigo-600"
    >
      <template v-slot:mainInfo>
       {{ clientStore.clients_paid_off }}
      </template>

      <template v-slot:subInfo>
        Clientes quitados
      </template>
    </CardTop>
  </div>

    <div class="chart bg-white lg:col-span-3 xl:col-span-5 border border-gray-200 mt-3">
    <TableClients/>
  </div>
</template>

<script setup>
import CardTop from '@/components/dashboard/CardTop.vue';
import TableClients from '@/components/dashboard/TableClients.vue';

import { useClientStore } from '@/stores/client';
import { onMounted } from 'vue';
const clientStore = useClientStore();

onMounted(() => {
  clientStore.getStatistics();
});
</script>