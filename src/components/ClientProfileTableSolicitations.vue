<template>
  ...
  <div v-else class="bg-white shadow rounded overflow-x-auto">
        ...
        <tr v-for="solicitation in clientProfile?.solicitations" :key="solicitation.id" class="hover:bg-gray-50">
          ...
          <td class="px-6 py-4 relative">
            ...
            <DropSolicitationTable :solicitation="solicitation" @deleteSolicitation="deleteSolicitation"
              @approveSolicitation="approveSolicitation" @counterofferSolicitation="counterofferSolicitation"
    ...
    <BaseModal v-model="recuseSolicitationModal.isOpen.value" modalTitle="Recusar solicitação?"
      ...
      confirmText="Sim, recusar" :loading="recuseSolicitationModal.loading.value">
...
const submit = handleSubmit((values) => {
  ...
  return solicitationStore.counterofferSolicitation(values, selectedId)
    .then(() => {
      ...
      solicitationSelected.value.counteroffer = values.counteroffer
      solicitationSelected.value.tax = values.tax
      solicitationSelected.value.status = 'Contra-oferta'
...
function confirmDeleteSolicitation(id) {
  ...
  const index = clientStore.clientProfile.solicitations.findIndex(solicitation => solicitation.id === id);
  ...
  return solicitationStore.deleteSolicitation(id)
    .then(() => {
      ...
      if (index >= 0) {
        ...
        clientStore.clientProfile.solicitations.splice(index, 1);
...
function confirmApproveSolicitation(id, amount) {
  ...
  return solicitationStore.approveSolicitation(id, amount)
    .then(() => {
      ...
      solicitationSelected.value.status = 'Aprovada'
